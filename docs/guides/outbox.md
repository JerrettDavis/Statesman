...
- **The filesystem change-log fsync gap.** The change-log append is never fsynced, even at `FlushToDisk = true` — only the history and head writes are — so after a host or power failure every change-log line the OS had not yet flushed back can be missing while its history file survives durably. That window can span many records and many seconds, not just the one write between the history file and the change-log line; a process crash without a host or power failure does not widen it.
...
**Any provider with `StateRetentionPolicy.KeepAll` is a complete configuration for feed losslessness**, with the two residuals above still standing: the filesystem provider's fsync gap regardless of which provider backs the outbox's own cursor store, and the lease residual specifically for a filesystem- or in-memory-backed outbox. See the providers page's [change feed semantics and limitations](../providers/index.md#change-feed-semantics-and-limitations) for the per-provider mechanism, and its [outbox delivery semantics and limitations](../providers/index.md#outbox-delivery-semantics-and-limitations) for the per-provider breakdown.
