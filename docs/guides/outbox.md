...
That is a promise about the dispatch loop, not about the ledger, and the two are not the same thing. Composed with the feed the outbox reads from, it becomes at-least-once delivery of every record *the feed yields* — which is narrower than "every committed change is delivered," for two reasons the outbox inherits and cannot fix:

- **Tail loss.** The in-memory, filesystem, and Redis providers allocate a record's global position before the record is durably visible, under a per-address gate only. A record whose position was allocated before, but committed after, a later record the outbox already consumed is permanently skipped — the outbox has already advanced its cursor past that position and will never see the record land.
- **Prune loss.** `StateHandle` prunes after every successful append, so on Entity Framework Core with any non-default retention policy a record can leave the feed's own table milliseconds after being written, before the outbox ever reads it.

**The one complete configuration is Entity Framework Core as the feed source with `StateRetentionPolicy.KeepAll`.** There the position and the record commit in one serializable transaction, and nothing is pruned. Every other provider, and Entity Framework Core with any other retention policy, inherits one or both of the caveats above. See the providers page's [change feed semantics and limitations](../providers/index.md#change-feed-semantics-and-limitations) for the underlying mechanism, and its [outbox delivery semantics and limitations](../providers/index.md#outbox-delivery-semantics-and-limitations) for the per-provider breakdown. Feed hardening — commit-time position allocation, or a settle window before a record is considered readable — is the named prerequisite for a stronger promise, and is a phase of its own.
...
Set `RequireLease = false` to run unleased anyway. This is a documented degradation, safe only when exactly one process will ever dispatch this outbox, and the hosted worker logs a warning once at start naming the store and outbox id. Redis and Entity Framework Core are the only shipped providers that implement `IStateLeaseProvider`, so an outbox over the filesystem or in-memory provider needs `RequireLease = false` or a store-supplied lease provider of its own. The outbox worker is this repository's first production caller of `IStateLease.RenewAsync` — it renews the lease while draining a long feed rather than letting it lapse mid-cycle.
...
Cursors are keyed per store *and* per outbox id: two outboxes reading the same store need distinct `OutboxId` values, and pointing one outbox id at two different stores corrupts its resume point. `InMemoryOutboxCursorStore` is correct for tests and single-process development only — it loses its position on restart, so the outbox re-delivers its entire backlog after every process restart. `FileSystemOutboxCursorStore` persists to one JSON file per outbox id, written atomically (temp file, then move), and enforces monotonicity under a gate keyed by the resolved file path shared across instances in one process — it is not safe for multiple processes writing a shared network directory, which is exactly what `RequireLease = true` exists to prevent by keeping only one process dispatching at a time.
