...
That is a promise about the dispatch loop, not about the ledger, and the two are not the same thing. Composed with the feed the outbox reads from, it becomes at-least-once delivery of every record *the feed yields*. The feed is lossless within retention on every provider — a consumer resuming from the cursor of the last record it accepted is never skipped past a record — so that is narrower than "every committed change is delivered" for one reason the outbox inherits and cannot fix, plus one provider-specific residual — listed together below, because the two are easy to confuse:

- **Prune loss.** `StateHandle` prunes after every successful append, so with any non-default retention policy a record can leave the feed milliseconds after being written, before the outbox ever reads it. This is uniform: a record leaves the change feed exactly when its history record leaves the store, on every provider. No provider trades that loss for unbounded feed growth any more — the filesystem provider's change log and its in-process index still grow with total writes; see the providers page — Redis and the in-memory provider used to, and `MaxRevisions`/`MaxBytes` now bound their feeds as well as their histories, which is what those options always claimed to do.
...
**Any provider with `StateRetentionPolicy.KeepAll` is a complete configuration for feed losslessness** — and `KeepAll`, the default, is now the only way to avoid prune loss on any provider — with one residual still standing: the lease residual for a filesystem- or in-memory-backed outbox. See the providers page's [change feed semantics and limitations](../providers/index.md#change-feed-semantics-and-limitations) for the per-provider mechanism, and its [outbox delivery semantics and limitations](../providers/index.md#outbox-delivery-semantics-and-limitations) for the per-provider breakdown.
