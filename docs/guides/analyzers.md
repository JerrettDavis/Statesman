...
A deconstructing assignment into a managed member is reported once per element: `(state.Scalar, _) = (1, 0)` reports, once per element that resolves to a property or field owned by managed state, nested tuples included. The declaration forms are not: `var (a, b) = (1, 2)` and `(int c, int d) = (3, 4)` do not report, because those declare locals, and a local is not owned by managed state. The rule is the same ownership walk STM001 already used; only which nodes reach it has widened.
