...
Per-provider suitability as an outbox source follows directly from "Change feed semantics and limitations" above. Entity Framework Core with `StateRetentionPolicy.KeepAll` is the one complete configuration: the position and the record commit together and nothing is pruned. In-memory, filesystem, and Redis all inherit the feed's tail loss, because all three allocate a record's global position before the record is durably visible under a per-address gate only. Entity Framework Core with any configured (non-`KeepAll`) retention policy inherits prune loss instead, because `StateHandle` prunes after every successful append and the feed queries the same table that prune deletes from. The tiered provider reads its change feed from its cold tier and takes its outbox lease hot-first, so an outbox over a tiered store must key its cursor by the tiered store's own name, not its cold store's — the two are different `IStateLedgerStore.Name` values with unrelated cursor positions.
