...
Redis implements `IStateLedgerReplica`, so it can serve as a tiered hot replica or as a restore target for `Statesman.Tooling`. An import writes the exact record — same revision, same global position — into the head, revision guard, history set, change feed, and partition hash in one server-side script, replacing any member that already holds that revision or position rather than duplicating it. First, it raises the store's global position counter to at least the imported position, so a later append never reuses an imported position. Imports are refused above global position 2^52 (`RedisStateLedgerStore.MaxImportablePosition`) because sorted-set scores are IEEE doubles, exact only to 2^53 — a filesystem export, whose positions are UTC ticks, cannot be restored into Redis. `ImportAsync` performs every read and write of one import inside a single server-side script, so two concurrent imports of the same revision cannot interleave between the read of the stale feed member and its replacement: the guarantee is the script's atomicity, not a retry, and it holds on a cluster under `RedisKeyLayout.SingleSlot`.

## Entity Framework Core

Provides a base `StatesmanLedgerDbContext`, head table, append table, and sequence table. Create a derived context, register an `IDbContextFactory<TContext>`, and select the store by name — then either add your own migration, or take one of the three shipped `Statesman.Persistence.EntityFrameworkCore.{Sqlite,SqlServer,PostgreSQL}` migration packages (see [Entity Framework Core migrations](entity-framework-core-migrations.md)).

```csharp
services.AddPooledDbContextFactory<AppStateDbContext>(options =>
    options.UseSqlServer(connectionString).UseStatesmanLedgerSqlServerMigrations());
...
truncation or a compaction safe. A `_changes.gen` sidecar adds a generation the index checks before
and after each refresh — a compaction bumps it before its rename — so a same-length rewrite of an
earlier line, which the tail check alone cannot see, forces a reparse instead of silently trusting
stale bytes; the cost is two small file reads per feed read, and a missing sidecar reads as
generation zero, so no existing store directory needs migrating. A generation mismatch forces a
full-log reparse instead, at most `MaxChangeFeedGenerationRetries + 1` (4) times per `ReadAsync`
even with a compactor running between every read. It is per process: a second process
reading the same directory pays its own first parse.
