...
/// <summary>
/// Fails when <see cref="StatesmanOutboxCursorDbContext"/>'s model no longer matches a shipped
/// migration.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
public sealed class EntityFrameworkOutboxMigrationDriftTests
{
    [Fact]
    public void The_sqlite_migration_matches_the_ledger_model() =>
        AssertNoDrift(builder => builder
            ...
            .UseStatesmanOutboxSqliteMigrations());
    ...
    public void The_sql_server_migration_matches_the_ledger_model() =>
        AssertNoDrift(builder => builder
            ...
            .UseStatesmanOutboxSqlServerMigrations());
    ...
    private static void AssertNoDrift(Action<DbContextOptionsBuilder<StatesmanOutboxCursorDbContext>> configure)
    {
        var builder = new DbContextOptionsBuilder<StatesmanOutboxCursorDbContext>();
        configure(builder);
        using var context = new StatesmanOutboxCursorDbContext(builder.Options);
        ...
        Assert.False(
            ...
            "StatesmanOutboxCursorDbContext's model has drifted from this package's shipped migration. "
